...
function getTempFolderName() {
  return process.env.BOX_TEMP_FOLDER_NAME || "Temp";
}
...
export async function ensureTempFolder() {
  return ensureFolderByName(getTempFolderName());
}
...
export async function getTempUploaderConfig() {
...
export async function finalizeRaffleUpload(fileId, values) {
  const client = getBoxClient();
  const tempFolder = await ensureTempFolder();
  ...
  if (uploadedFile?.type !== "file" || uploadedFile?.parent?.id !== tempFolder.id) {
    throw new BoxRequestError(
      "This file is not in the configured Temp folder.",
      403,
    );
  }

  const uniqueName = buildUniqueFileName(uploadedFile.name);
  const renamedFile = await client.files.updateFileById(fileId, {
    requestBody: {
      name: uniqueName,
    },
    queryParams: {
  ...
  const movedFile = await client.files.updateFileById(fileId, {
    requestBody: {
      parent: { id: raffleFolder.id },
    },
    queryParams: {
      fields: ["id", "type", "name", "parent"],
    },
  ...
  return {
    file: {
      id: movedFile.id,
      name: movedFile.name || renamedFile.name || uniqueName,
      parentId: movedFile.parent?.id,
    },
    metadata,
  };
}
