...
export async function ensureRaffleFolder() {
  return ensureFolderByName(getRaffleFolderName());
}

async function ensureFolderByName(folderName) {
...
export async function getRaffleUploaderConfig() {
...
function readRaffleMetadataFields(metadataLike) {
  ...
  return {
    ...
    email: String(raw?.email ?? extra.email ?? "").trim(),
  };
}

function metadataFromFolderItemFile(entry) {
  const props = entry?.metadata?.extraData?.global?.properties;
  return readRaffleMetadataFields(props);
}
...
export async function listRaffleFolderFiles() {
  ...
  while (true) {
    ...
    for (const entry of entries) {
      ...
      let { firstName, lastName, email } = metadataFromFolderItemFile(entry);
      ...
      if (!firstName && !lastName && !email) {
        try {
          ...
          ({ firstName, lastName, email } = readRaffleMetadataFields(md));
        } catch {
...
async function assertRaffleFolderFile(client, fileId, folder) {
  const file = await client.files.getFileById(fileId, {
    queryParams: {
      fields: ["id", "type", "name", "parent", "shared_link"],
    },
  });
...
function readFileSharedLink(file) {
  return file?.sharedLink ?? file?.shared_link ?? null;
}

function getSharedLinkUrl(sharedLink) {
  return sharedLink?.url ?? "";
}

function isPublicSharedLink(sharedLink) {
  if (!sharedLink?.url) {
    return false;
  }
  ...
  const isOpen =
    ...
    sharedLink.effectiveAccess === "open" ||
    sharedLink.effective_access === "open";
  const canDownload =
    sharedLink.permissions?.canDownload === true ||
    sharedLink.permissions?.can_download === true;

  return isOpen && canDownload;
}

export async function ensurePublicSharedLinkForRaffleFile(fileId) {
  const client = getBoxClient();
  const folder = await ensureRaffleFolder();
  const file = await assertRaffleFolderFile(client, fileId, folder);
  const existingSharedLink = readFileSharedLink(file);

  if (isPublicSharedLink(existingSharedLink)) {
    return getSharedLinkUrl(existingSharedLink);
  }

  const sharedLinkMethod = existingSharedLink
    ? client.sharedLinksFiles.updateSharedLinkOnFile.bind(client.sharedLinksFiles)
    : client.sharedLinksFiles.addShareLinkToFile.bind(client.sharedLinksFiles);
  const updated = await sharedLinkMethod(
    fileId,
    {
      sharedLink: {
        access: "open",
  ...
  const sharedLinkUrl = getSharedLinkUrl(readFileSharedLink(updated));

  if (!sharedLinkUrl) {
    throw new BoxRequestError(
      "Box created the shared link but did not return its URL.",
      502,
    );
  }
...
export async function listRaffleFolderFilesForCsvExport() {
  const rows = await listRaffleFolderFiles();
  const rowsWithSharedLinks = [];

  for (const row of rows) {
    const sharedLink = await ensurePublicSharedLinkForRaffleFile(row.id);

    rowsWithSharedLinks.push({
      ...row,
      sharedLink,
    });
  }

  return rowsWithSharedLinks;
}

export function formatRaffleEntriesCsv(rows) {
  const header = ["First Name", "Last Name", "Email", "Shared Link"];
  const lines = [
    header.map(escapeCsvField).join(","),
    ...rows.map((row) =>
      [row.firstName, row.lastName, row.email, row.sharedLink]
        .map(escapeCsvField)
        .join(","),
    ),
  ];
...
export async function deleteAllRaffleFolderFiles() {
  const client = getBoxClient();
  const folder = await ensureRaffleFolder();
  const fileIds = [];
  let offset = 0;
  ...
  while (true) {
    const body = await client.folders.getFolderItems(folder.id, {
      queryParams: {
        fields: ["id", "type"],
        limit,
        offset,
...
export async function finalizeRaffleUpload(fileId, values) {
  const client = getBoxClient();
  const raffleFolder = await ensureRaffleFolder();
  const uploadedFile = await client.files.getFileById(fileId, {
    queryParams: {
      fields: ["id", "type", "name", "parent"],
    },
  });

  if (uploadedFile?.type !== "file" || uploadedFile?.parent?.id !== raffleFolder.id) {
    throw new BoxRequestError(
      "This file is not in the configured Raffle folder.",
      403,
    );
  ...
  const metadata = await applyRaffleMetadata(fileId, values);
  const sharedLink = await ensurePublicSharedLinkForRaffleFile(fileId);

  return {
    file: {
      id: uploadedFile.id,
      name: uploadedFile.name,
      parentId: raffleFolder.id,
    },
    metadata,
    sharedLink,
  };
